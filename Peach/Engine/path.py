...
class PathValidator:
    ...
    def validate(self):
		...
		if not lastPath.stop:
			...
			if lastState.getChoice() != None:
				raise PathException("PathValidator: Invalid path definition. Final state[%s] must not contain Choice element." % lastState.name)
		...
		while self.pathFinder.canMove():
			...
			if not self._isNextState(thisState.stateName, nextState.stateName):
				raise PathException("PathValidator: Invalid path definition at StateMachine[%s]. State[%s] does not contain state[%s] in its choice list." % (self.pathFinder.stateMachine.name, thisState.stateName, nextState.stateName))
		...
		if len(self.validationMutator.states) < len (self.pathFinder.getRoute()):
			raise PeachException("Invalid path. StateMachine[%s] violated path definition declared in XML file." % self.pathFinder.stateMachine.name)
    ...
    def _isNextState(self, stateName, nextStateName):
		...
		if choice != None:
			if choice.findActionByRef(nextStateName) != None:
				ret = True
		else:
